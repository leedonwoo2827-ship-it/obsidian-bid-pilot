...
| 명령어 | 설명 | 추천 단축키 |
|---|---|---|
| 수주 AI 채팅 열기 | 우측 사이드바 채팅 뷰 | `Ctrl+Shift+K` |
| 선택 영역을 채팅으로 질문 | 선택 텍스트 → 채팅 자동 전송 | `Ctrl+Shift+Enter` |
| 선택 영역을 컨텍스트로 핀 | 선택 텍스트를 근거로 핀 | `Ctrl+Shift+L` |
| 마지막 AI 응답을 커서 위치에 삽입 | AI 응답 → 에디터 삽입 | `Ctrl+Shift+I` |
| 채팅 대화 초기화 | 히스토리 삭제 | — |
| 채팅 모델 전환 (Flash ↔ Pro) | 무료 티어 전략용 | `Ctrl+Shift+M` |
| 컨텍스트 매니저 열기 | 좌측 사이드바 | `Ctrl+Shift+C` |
| 현재 파일 AI 분석 | Gemini로 분석, 프로퍼티 생성 | `Ctrl+Shift+A` |
| 컨텍스트 전체 분석 | `_context/` 일괄 분석 | — |
| 컨텍스트 볼트 재인덱싱 (RAG) | 벡터 DB 재생성 | — |
